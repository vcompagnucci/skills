...
- **Stateless requests can still cache.** Codex resends the full history instead of pointing at conversation state stored on the server, to keep requests stateless and serve zero-data-retention customers. The stable prefix is what keeps that affordable. (`agent-loop`)
