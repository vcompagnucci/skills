...
- **When the charge is uncertain, fail closed and keep the money held.** A timed-out request may still run and bill, so the controller blocks the run for good and keeps its reservation unavailable. The same goes for missing or inconsistent usage, an unexpected model, or a cost above the hold. Automatic client retries are off, because a retry spends money nobody reserved. (`spend-controller`)
