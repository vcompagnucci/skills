...
- **Is the position contested or did it change?** State it as the authors' position, unsoftened, with the dates. The known ones: effort first vs a stronger model first, examples in tool definitions (2025) vs interface design (Claude 5), a single agent with skills vs subagents, multi-agent cost (3-10x vs a cheaper planner-worker split), how much structure to impose on agent teams, whether a prompt rule can gate a destructive action, and owning your harness vs Managed Agents.
