...
- **Monitors that read the reasoning catch more, but aren't deployment-grade yet.** In SHADE-Arena (research, 17 tasks with a hidden sabotage side task), most models revealed the side task in their own reasoning. The best monitor needed a false-positive rate over 20% to catch 80%, though it still beat humans who spent 1.5 hours per transcript. (`shade-arena`)
...
- **Split the job into tasks and set a number for each before launch.** The support guide decomposes a conversation into greeting, product info, staying on topic, and quoting, each with its own test cases, then sets targets such as 100% on static product facts and 95%+ escalation accuracy. Routing targets 95%+ consistency and 90%+ accuracy within 50-100 samples of a new category (full lists in `agents-in-production.md`). (`uc-support`, `uc-routing`)
...
- **Noisy evals miss real regressions.** A 2025 infrastructure bug slipped past evals partly because Claude recovers well from isolated mistakes. A 2026 prompt line limiting words between tool calls passed weeks of testing, but ablations showed a 3% drop. Run evals continuously on production, ablate prompt changes, and roll out gradually. A worked example: a routing rule that fixed API tickets regressed billing ones, caught only by scoring both prompt versions on a labelled set, so promote by re-pinning a version rather than editing live. (`postmortem-sep-2025`, `postmortem-apr-2026`, `cb-prompt-versions`)
...
## Key source articles
`agent-evals` · `harness-design-apps` · `cc-best-practices` · `research-system` · `infra-noise` · `eval-awareness` · `c-compiler` · `effort` · `cb-outcome-grader`
