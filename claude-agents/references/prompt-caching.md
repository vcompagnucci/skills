...
- **Cache first, because it costs no quality.** The cost cookbook pulls caching before every lever that touches intelligence. In the Agent SDK it needs no setup: the system prompt and tool schemas cache after the first call, and later calls pay about 10% of input for that prefix. (`cb-cost`, `cb-openai-migration`)
