...
How Anthropic keeps agents from doing damage as they get more autonomous. The core idea across the engineering, security, and research posts, with cookbooks as worked examples, is that the chance an agent fails keeps falling while what it could break keeps growing. So cap the blast radius with deterministic boundaries first and use prompts, classifiers, and human approvals as extra layers on top.
...
- **Layered injection defenses still leave risk.** In browser use, Anthropic still calls Opus 4.5 at 1% attack success against an adaptive attacker "meaningful risk", so RL training, a classifier on all untrusted content, and human red-teaming run together. (`injection-defenses`)
- **Oversight spans four layers, and the model is only one.** The layers are model, harness, tools, and environment, and "a well-trained model can still be exploited through a poorly configured harness, an overly permissive tool, or an exposed environment." (`trustworthy-agents`)
...
- **Prompts are suggestions, so move must-dos into the harness.** Over a long run, "every word will likely be ignored eventually". Guardrails in the harness also free up context. (`outtake`, `commerce-agents`)
...
- **The agent proposes, and approvals happen outside the chat.** "The model's most dangerous action is to propose." Every merchant write gets a server ID and preview. Limits (max price move, promo depth, budget) are checked when staged and again when applied, and only a button, console, or always-ask policy applies the write. "An approval typed in chat approves nothing." Cart writes accept only product IDs a tool returned that session. (`commerce-agents`, `uc-commerce`)
...
- **An egress allowlist grants capabilities rather than filtering destinations.** Every function reachable through an allowed domain is attack surface. (`containment`)
- **Keep credentials structurally out of reach.** Git tokens are wired into the remote at setup and MCP tokens sit in a per-end-user vault behind a proxy, so generated code never sees them. The agent reaches a database through a host-side custom tool, since a connection string in a cloud sandbox lands in session history. Assume a prompt-injected model will read its own environment. (`managed-agents`, `agentic-surfaces`, `sandboxing`, `cb-production`, `cb-mongodb`)
...
- **Check that a restriction actually restricts.** `allowed_tools` only governs approval prompts, so an agent scoped to GitHub MCP still shells out to `gh` unless `disallowed_tools=["Bash"]`. The Agent SDK permissions docs confirm this as of 2026-09-26. Unlisted tools stay available and fall through to the permission mode, and only a bare-name `disallowed_tools` entry removes a tool from context. `Read` rules cover `Grep` and `Glob` only best-effort, so an unattended reviewer adds a `PreToolUse` hook that denies paths outside the repo. (`cb-observability`, `cb-repo-reviewer`)
