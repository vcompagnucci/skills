...
- **In Claude Code, the context window is the most important resource.** Performance degrades as it fills, and one debugging session can use tens of thousands of tokens. After correcting the same issue twice, `/clear` and restart with a better prompt: "a clean session with a better prompt beats a long one full of corrections". (`cc-best-practices`)
- **Prompting is converging with context engineering.** For Claude 5 generation models: "less scaffolding, more curation". Instructions for every session belong in CLAUDE.md, skills, or other steering. (`prompt-engineering`)

## Delete most of it (Claude 5)

- **The team removed over 80% of Claude Code's system prompt for Opus 5 and Fable 5,** with no measurable loss on coding evals. `/doctor` trims your skills and CLAUDE.md the same way. (`ctx-eng`)
