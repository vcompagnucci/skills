...
- **Descriptions are prompts, and they carry more weight than schemas.** Describe the tool as to a new hire and name parameters unambiguously (`user_id`, not `user`). Precise description edits took Claude 3.5 Sonnet to SWE-bench state of the art. A description fix stopped Claude from appending "2025" to every web search. An SRE agent built the right PromQL query from the description alone. Descriptions should also say what the tool returns. (`writing-tools`, `swe-bench`, `cb-sre-agent`, `cb-threat-intel`, `cb-support-agent`)
...
- **Scripts beat traditional tools for many jobs.** Code is self-documenting and doesn't sit in context. When Claude kept rewriting the same slide-styling script, the team had it save the script as a tool for itself. (`skills-for-agents`)
...
- **Give write access through narrow, checked tools, not a shell.** The SRE agent's config editor only writes under `config/`, its shell only runs `docker` commands, and a `PreToolUse` hook rejects a `DB_POOL_SIZE` outside 5-100, so the check covers what changes as well as where. Investigation runs read-only, and remediation waits for a separate authorization. (`cb-sre-agent`)
