...
- **Most teams use multi-agent where one agent would do better.** Better prompting on a single agent matched months of elaborate architecture, and multi-agent typically costs 3-10x the tokens for the same task. (`when-multi-agent`)
...
- **Scope each specialist's tools to its role.** The scope is a containment boundary. The pricer can't pull a competitor's number off the web, and the case-study picker's hundreds of file reads stay out of the coordinator's context. The coordinator sequenced dependent hand-offs without being told the order. Set `effort` per role too (high for judgment-heavy research). (`cb-specialist-team`, `cb-watch-subagents`)
- **The orchestrator is an information bottleneck.** When one subagent's finding matters to another, the orchestrator summarizes it away as it passes through. Writing artifacts to a filesystem and passing references avoids the telephone game. (`coordination-patterns`, `research-system`)
...
- **Groups fail when the decisive fact sits with one agent.** Trusting a lone dissenter over apparent consensus scaled with intelligence without saturating. The post's proposed root cause is that agents have no reputation or memory of past partners. (`multiagent-problems`)

## Where the answer depends on the case

- **The posts on how much structure to impose describe different tasks.** `cb-dynamic-workflows` (2026-07-22) argues the script's structure, not a smarter model, is what makes verification reliable. `auto-alignment` (2026-04-14) found that a rigid propose-plan-code workflow constrained nine Opus 4.6 research agents, compared with letting them design their own cheap-experiment-then-commit process. Without distinct starting directions, they converged and progressed less.
